"""LSTM extractor (FR-MOD-4 / PRD_lstm_model).

Standard 4-gate LSTM cell (forget, input, candidate, output). Per-timestep
input ``[sample, one_hot]`` (ADR-003). Last hidden state → ``Linear`` head.
"""
...
@register(Architecture.LSTM.value)
class LSTMExtractor(BaseExtractor):
    """``torch.nn.LSTM`` (4 gates), final hidden state → ``Linear`` to 10."""
    ...
    def architecture_name(self) -> str:
        return Architecture.LSTM.value
