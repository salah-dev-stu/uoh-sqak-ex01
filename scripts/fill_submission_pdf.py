"""Fill the submission .docx template with the locked metadata and export to PDF.

Final values (locked before submission):
  group_code = "uoh-sqak"   (Salah Qadah + Andalus Kalash)
  self_grade = 90
  github     = https://github.com/salah-dev-stu/uoh-sqak-ex01
  late       = no

Run:  uv run python scripts/fill_submission_pdf.py
Outputs:
  uoh-sqak-ex01.docx   (filled cover sheet)
  uoh-sqak-ex01.pdf    (final; what gets uploaded to Moodle)
"""

from __future__ import annotations

import shutil
import subprocess
import sys
from pathlib import Path

from docx import Document

REPO = Path(__file__).resolve().parent.parent
TEMPLATE = REPO / "uoh-rl07-ex01.docx"

GROUP_CODE = "uoh-sqak"
SELF_GRADE = "90"
EXERCISE_NUMBER = "01"
GITHUB_URL = "https://github.com/salah-dev-stu/uoh-sqak-ex01"
LATE = "no"

STUDENT_1 = {
    "id": "323039974",
    "first_en": "Salah",
    "last_en": "Qadah",
    "first_he": "סלאח",
    "last_he": "קדח",
}
STUDENT_2 = {
    "id": "211435797",
    "first_en": "Andalus",
    "last_en": "Kalash",
    "first_he": "אנדלוס",
    "last_he": "כלש",
}


def append_value(paragraph, value: str) -> None:
    """Append " <value>" as a new run to a paragraph (preserves label run)."""
    paragraph.add_run(" " + value)


def main() -> int:
    if not TEMPLATE.exists():
        print(f"FAIL: template not found at {TEMPLATE}", file=sys.stderr)
        return 1
    intermediate = REPO / f"{GROUP_CODE}-ex01.docx"
    shutil.copy(TEMPLATE, intermediate)

    d = Document(intermediate)
    p = d.paragraphs

    append_value(p[0], EXERCISE_NUMBER)               # exercise number
    append_value(p[2], GROUP_CODE)                    # group ID code
    append_value(p[4], SELF_GRADE)                    # self-grade
    append_value(p[7], STUDENT_1["id"])               # S1 ID
    append_value(p[8], STUDENT_1["first_en"])
    append_value(p[9], STUDENT_1["last_en"])
    append_value(p[10], STUDENT_1["first_he"])
    append_value(p[11], STUDENT_1["last_he"])
    append_value(p[14], STUDENT_2["id"])              # S2 ID
    append_value(p[15], STUDENT_2["first_en"])
    append_value(p[16], STUDENT_2["last_en"])
    append_value(p[17], STUDENT_2["first_he"])
    append_value(p[18], STUDENT_2["last_he"])
    append_value(p[20], GITHUB_URL)                   # GitHub link
    append_value(p[21], LATE)                         # late yes/no

    d.save(intermediate)
    print(f"  filled docx: {intermediate}")

    pdf = REPO / f"{GROUP_CODE}-ex01.pdf"
    # xelatex is the only common engine that handles Hebrew (RTL + Unicode) cleanly
    # Lucida Grande includes Hebrew glyphs (and Latin); xelatex + the
    # mainfont/CJKmainfont combo renders both scripts inline cleanly.
    rc = subprocess.run(
        [
            "pandoc", str(intermediate), "-o", str(pdf),
            "--pdf-engine=xelatex",
            "-V", "mainfont=Lucida Grande",
            "-V", "geometry:margin=1in",
        ],
        capture_output=True, text=True,
    )
    if rc.returncode != 0:
        print("pandoc xelatex failed; STDERR:")
        print(rc.stderr)
        return 1
    print(f"  pdf written: {pdf}  ({pdf.stat().st_size:,} bytes)")
    return 0


if __name__ == "__main__":
    sys.exit(main())
